 ...
 *
 * The labels are words and not `1h`, `1d`: an hour is not abbreviated the same way in every language, and
 * seven buttons of hard-coded English above a German dashboard is exactly what nobody wants to see.
 */
const RANGES: { hours: number; label: string }[] = [
    { hours: 1, label: 'range_1h' },
    { hours: 6, label: 'range_6h' },
    { hours: 12, label: 'range_12h' },
    { hours: 24, label: 'range_1d' },
    { hours: 72, label: 'range_3d' },
    { hours: 168, label: 'range_7d' },
    { hours: 720, label: 'range_30d' },
];
...
export interface ChartSeries {
    ...
    step?: boolean;
    /** The line is drawn as a smooth curve; a step curve is never one. */
    spline?: boolean;
}
...
/**
 * The `d` of the line: straight from point to point, held until the next one, or a smooth curve.
 *
 * A step curve is never smoothed: a switch that went on at ten past does not slope up to it.
 *
 * @param points where the points sit on the drawing
 * @param step true when the value is held until the next point
 * @param spline true when the line is drawn as a curve
 * @returns the path
 */
function buildLinePath(points: { x: number; y: number }[], step?: boolean, spline?: boolean): string {
    if (points.length < 2) {
        return '';
    }
    if (step) {
...
function InteractiveChart(props: InteractiveChartProps): React.JSX.Element {
    ...
    for (let i = 0; i < series.length; i++) {
        ...
        const d = buildLinePath(points, s.step, s.spline);
        if (!d) {
            continue;
        }
...
export interface HistoryChartProps {
    ...
    obj2Step?: boolean;
    chartTitle?: string;
    /** How far back the chart looks when it opens, in hours; one of the periods of the buttons */
    hours?: number;
...
export default function HistoryChart(props: HistoryChartProps): React.JSX.Element {
    ...
    const [hours, setHours] = useState(props.hours || 24);
