...
function calcProject(
    ...
    instance: number,
    result: { id: string; val: number }[],
    callback: (err: Error | null | undefined, result: { id: string; val: number }[]) => void,
): void {
    if (!projects?.length) {
    ...
    void objects.readFile(
        `vis-2.${instance}`,
        `/${project.file}/vis-views.json`,
        (err: Error | null | undefined, data?: Buffer | string, _mimeType?: string): void => {
            let json: Project;
            try {
...
export default function calcProjects(
    objects: ObjectsClient,
    ...
    objects.readDir(
        ...
        '/',
        null,
        (err: Error | null | undefined, projects?: ioBroker.ReadDirResult[]) => {
