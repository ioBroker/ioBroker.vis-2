...
* (@typhosj) The entries under "Recently used" in the palette do not count as widgets of their widget set anymore. The GUI test of vis-2 - and of every widget set that uses `@iobroker/vis-2-widgets-testing` - read them as widget types named `recent_...` and failed to place them
...
* (@GermanBluefox) The widgets of the widget set `vis-2-widgets-material` that need no library of their own ship with vis-2 itself now: blinds, camera, clock, security, simple state, theme switcher, vacuum cleaner and washer/dryer. Their ids, their set and their translations stayed as they were, so the projects that use them keep working, and the set gives them up on its side. A widget an installed widget set brings under the name of a built-in one is ignored now - the built-in one wins
* (@GermanBluefox) The last four widgets of `vis-2-widgets-material` that could be freed of their libraries moved into vis-2 as well: thermostat, static information, lock and navigation. The history chart of the thermostat and of the static information is loaded when its dialog is opened, so echarts stays out of the start of the page, the lock draws its door itself instead of playing a Lottie animation, and the dialog that re-orders the entries of the navigation uses the drag and drop of the editor
* (@GermanBluefox) The dialog of the thermostat widget has two tabs now: the thermostat itself, at the size the dialog offers, and the history of its temperatures. Until now the dialog opened empty, as it only ever showed a chart that nothing switched to. Its button is there without a history as well, and the dialog then shows the thermostat alone
* (@GermanBluefox) The widgets that came from `vis-2-widgets-material` form a set of their own now: **Devices**, which stands in the palette before `basic`, since a page is usually built from them. An installed material set keeps its own tab with the widgets it still has, so the two do not mix anymore. The `tpl` of every widget stayed as it was, so stored projects keep working
